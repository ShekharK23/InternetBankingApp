...
public class TransactionServiceImpl implements ITransactionService {
	...
	public Transaction createTransaction(Transaction transaction) throws InvalidDetailsException {
		...
		else throw new InvalidDetailsException("Please fill all the required field in order to create a Transaction", TransactionServiceImpl.class+"");
	}
	...
	public Transaction withdraw(long accounId,double amount,Transaction t) throws InvalidAccountException,LowBalanceException, InvalidDetailsException {
		...
		if(existingAccount instanceof SavingsAccount) {
			...
			if(existingAccount.getBalance()<savings.getSavingMinBalance()) {
				throw new LowBalanceException("You Should maintain Minimum balance of "+savings.getSavingMinBalance(), TransactionServiceImpl.class+"");
			}
			else {
			...
			if (existingAccount.getBalance() < currentAcc.getCurrentMinBalance()) {
				throw new LowBalanceException("You Should maintain Minimum balance of "+currentAcc.getCurrentMinBalance(), TransactionServiceImpl.class+"");
			} else {
				existingAccount.setBalance(existingAccount.getBalance() + amount);
				t.setAmount(amount);
				t.setTransactionType(TransactionType.DEBIT);
